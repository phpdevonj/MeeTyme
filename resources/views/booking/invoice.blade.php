                    ...
                    <div class="table-responsive mt-20">
                        <table class="table mt-20">
                            <tbody>
                                ...
                                <tr>
                                    <td colspan="3"></td>   
                                    <td >{{__('messages.discount')}}({{$bookingdata->discount}}%)</span> </td>
                                    <td style="width: 20%;">-{{getPriceFormat($bookingdata->final_discount_amount)}}</td>
                                </tr>
                                <tr>
                                    <td colspan="3"></td>   
                                    <td >{{__('messages.Coupon_Discount')}}</span> </td>
                                    @php
                                                        $discount = '';
                                                        ...
                                                        @endphp
                                    <td style="width: 20%;">{{ optional($bookingdata->couponAdded)->code ?? '-' }} {{ $discount }}</td>
                                </tr>
                                <tr>
                                    <td colspan="3"></td>   
                                    <td >{{__('messages.discounted_price')}}</span> </td>
                                    <td style="width: 20%;">{{getPriceFormat($bookingdata->final_sub_total)}}</td>
                                </tr>
                                @php
                                    $taxes = json_decode($bookingdata->tax);
                                    $serviceTax = collect($taxes)->firstWhere('title', 'Service Tax');
                                    $calculatedTax = 0;
                                    if ($serviceTax) {
                                ...
                                @php
                                    $taxes = json_decode($bookingdata->tax);
                                    $tax21 = collect($taxes)->firstWhere('title', 'Tax');// Find the "Tax" item
                                    $calculatedTax21 = 0;
                                    if ($tax21) {
                                ...
                                <!-- <tr>
                                    ...
                                    @php
                                                        ...
                                                        $coupon_discount = $subTotalWithTax * (float)$discount / 100;
                                                        $discount = $subTotalWithTax * $bookingdata->discount / 100;
                                                        $total_amount = $subTotalWithTax - ($coupon_discount + $discount);
                                                    @endphp
                                ...
                                <!-- <tr>
                                    ...
                                    <td style="width: 20%;">{{!empty($total_amount) ? getPriceFormat($total_amount + $extraValue) : 0}}</td>                                   
                                </tr> -->
                    </tbody>
                </table>
            </div>
