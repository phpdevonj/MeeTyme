...
function get_provider_commission($bookings) {
    $final_total_tax = 0;
    $all_final_total_tax = $bookings->map(function ($booking) {
        return $booking->final_total_tax ?? 0;
    })->toArray();
    $all_admin_earning = $bookings->map(function ($booking) {
        return $booking->total_admin_fee ?? 0;
    })->toArray();
    ...
    $total_amount = $total;
    $final_total_tax = array_sum($all_final_total_tax);
    $final_admin_earning = array_sum($all_admin_earning);
    return [
        ...
        'final_admin_earning' => $final_admin_earning,
    ];
}
...
function adminEarning() {
    $revenuedata = \App\Models\Payment::selectRaw('sum(total_amount) as total , booking_id, DATE_FORMAT(datetime , "%m") as month')
        ...
        ->where('payment_status', ['paid', 'advanced_paid'])
        ->groupBy('month');
...
function getMonthlyAdminEarning() {
    $monthlyEarnings = array_fill(1, 12, 0);

    $bookings = \App\Models\Booking::selectRaw('MONTH(created_at) as month, SUM(total_admin_fee) as admin_fee')
    ...
    foreach ($bookings as $booking) {
        $monthlyEarnings[(int)$booking->month] = (float)$booking->admin_fee;
    }

    return array_values($monthlyEarnings);
}


function getDailyAdminEarning($date) {
    $providers = \App\Models\User::where('user_type', 'provider')->get();
    $dailyEarning = 0;
    ...
    foreach ($providers as $provider) {
        ...
        if ($bookings->count() > 0) {
            ...
            $dailyEarning += $booking_data['final_admin_earning'];
        }
    }
...
function lenguageFileNames() {
    $enDir = resource_path('lang/en'); // default language
    $fileNames = [];
...
function createUpdateCategoryLang(): void {
    ...
    $langDir = resource_path('lang/');
    $enDir = $langDir . 'en';
    $dirToBeCreate = 'categorylang.php';
    $fileName = $enDir . "/" . $dirToBeCreate;

    // 4. Ensure the target directory exists (e.g., resource/lang/en)
    if (!File::isDirectory($enDir)) {
    ...
    foreach ($categories as $category) {
        $langData[strtolower(str_replace(' ', '_', $category->name))] = $category->name;
    }

    $arrayContent = var_export($langData, true);
    $phpContent = "<?php\n\nreturn " . $arrayContent . ";\n";
    File::put($fileName, $phpContent);

    // remove language key which are not available anymore.
    $langDirPath = resource_path('lang');
    if (!File::isDirectory($langDirPath)) {
        return;
    }
    $directoriesPath = File::directories($langDirPath);
    $directoryNames = [];
    foreach ($directoriesPath as $directoryPath) {
        $directoryNames[] = basename($directoryPath);
    }

    $directoryNames = array_diff($directoryNames, ['en']);


    foreach ($directoryNames as $languageCode) {
        $langDir = resource_path('lang/' . $languageCode);
        $filePath = $langDir . '/' . $dirToBeCreate;
        ...
        $currentLangData = [];
        if (File::isFile($filePath)) {
            $currentLangData = File::getRequire($filePath);
        }

        $newLangData = [];
        foreach ($langData as $key => $value) {
            $newLangData[$key] = $currentLangData[$key] ?? $value;
        }
        $arrayContent = var_export($newLangData, true);
        $phpContent = "<?php\n\nreturn " . $arrayContent . ";\n";
...
function createUpdateSubCategoryLang(): void {
    $categories = SubCategory::select('name')->get();
    if (!$categories) {
        return;
    }
    ...
    $langDir = resource_path('lang/');
    $enDir = $langDir . 'en';
    $dirToBeCreate = 'subcategorylang.php';
    $fileName = $enDir . "/" . $dirToBeCreate;
...
function createUpdateServiceLang(): void {
    $categories = Service::select('name')->get();
    if (!$categories) {
        return;
    }
