...
class HomeController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $data['dashboard'] = [
            'count_last3DaysUsers'=> User::where('user_type','user')->where('created_at', '>=', Carbon::now()->subDays(3))->count(),
            'count_total_booking' => Booking::myBooking()->count(),
            'count_total_accept_booking' => Booking::myBooking()->where('status', 'accept')->count(),
            'count_total_cancelled_booking' => Booking::myBooking()->where('status', 'cancelled')->count(),
            'count_total_completed_booking' => Booking::myBooking()->where('status', 'completed')->count(),
        ...
        $data['total_revenue'] = $booking_data['final_admin_earning'];
        // end admin earning

        // total revenue not only admin
        // $total_revenue = Payment::whereIn('payment_status', ['paid','advanced_paid']);
        if (auth()->user()->hasAnyRole(['admin', 'demo_admin'])) {
            // $data['revenueData'] = adminEarning();
            $data['revenueData'] = getMonthlyAdminEarning();
        }
        ...
        if ($user->hasRole('provider')) {
            $total_revenue = ProviderPayout::where('provider_id', $user->id)->where('status','completed')->sum('amount') ?? 0;
    ...
    public function stripeReturn(Request $request) {
        ...
        // Get Stripe keys from your PaymentGateway model
        ...
        if ($account->charges_enabled && $account->payouts_enabled) {
            ...
            $status = 'incomplete';
            $message = '⚠️ Onboarding not yet complete. You can return later to finish setup.';
        }
    ...
    public function reauthView(Request $request) {
        $accountId = $request->query('account_id');
        if (!$accountId) {
            abort(404, 'Account ID missing');
        }
        $paymentGateway = PaymentGateway::where('type', 'stripe')->first();
        ...
        return view('stripe.reauth', compact('account'));
    ...
    public function startOnboarding(Request $request) {
        ...
        if (!$accountId) {
            return response()->json(['status' => 400, 'error' => 'No account ID provided.'], 400);
        }
        $account = Account::retrieve($accountId);
        if ($account->charges_enabled && $account->payouts_enabled) {
            return response()->json(['status' => 200, 'message' => 'Account is already onboarded.'], 200);
        }
    ...
    public function getRevenueData(Request $request) {
        ...
        if ($period === 'month') {
            ...
            if ($user->hasRole('admin')) {
                $revenuedata = \App\Models\Booking::selectRaw('SUM(total_admin_fee) as total, YEAR(created_at) as year')
                ...
                $revenuedata = ProviderPayout::selectRaw('SUM(amount) as total, YEAR(created_at) as year')
                ->where('provider_id', $user->id)
                ->where('status', 'completed')
                ->groupBy('year')
                ->orderBy('year')
            ...
            $categories = $revenuedata->pluck('year')->toArray();
            $data = $revenuedata->pluck('total')->map(fn($t) => (float)$t)->toArray();
        } elseif ($period === 'custom' && $startDate && $endDate) {
            // Custom date range (daily)
            ...
            while ($start->lte($end)) {
                ...
                if ($user->hasRole('admin')) {
                    $revenue = getDailyAdminEarning($start->format('Y-m-d'));
                } elseif ($user->hasRole('provider')) {
                    $revenue = ProviderPayout::where('provider_id', $user->id)
                        ->where('status', 'completed')
