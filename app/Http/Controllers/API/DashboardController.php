...
class DashboardController extends Controller
{
    ...
    public function providerDashboard(Request $request){
        ...
        $upcomming = Booking::where('provider_id',auth()->user()->id)
            ->where('status','accept')
            ->orderBy('id','DESC')
            ->count();

        $today_booking = Booking::where('provider_id',auth()->user()->id)
            ->whereDate('date', Carbon::today())
            ->count();

        $cancelled_booking = Booking::where('provider_id',auth()->user()->id)
            ->where('status','cancelled')
            ->orderBy('id','DESC')
            ->count();

        $completed_booking = Booking::where('provider_id',auth()->user()->id)
        ...
        $stripeDetails = $this->getStripeAccountDetails($provider->stripe_account_id,$provider->country_id);
        ...
        $response = [
            ...
            'admin_status'   => $provider->status,
            'total_booking'  => $total_booking,
            'accepted_booking'  => $upcomming,
    ...
    public function getStripeAccountDetails($stripeAccountId,$countryId){
        $response = [
            'currencySymbol' => '',
            'account' => null,
            'accountUrl' => null,
        ...
        try {
            ...
            if (empty($stripeAccountId)) {
                ...
                $countryCode = $countryId->country->code ?? 'NL';

                // Country restrictions
                if (in_array($countryCode, ['IN'])) {
                    return [
                        'status'       => 'not_connected',
                ...
                $accountData = [
                    'type' => 'custom', // or 'standard' depending on your flow
                    'country' => $countryCode,   // or 'US', etc. based on your business
                    'email' => auth()->user()->email, // or other user email
                    'capabilities' => [
            ...
            $response['currency']  = strtoupper($account->default_currency ?? 'EUR');
